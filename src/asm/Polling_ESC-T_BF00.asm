...
; Er kehrt danach zur�ck und schaltet die "normale" CAOS-Duplex-Routine (1200 Baud) wieder ein
;
...
; Nach der Daten�bertragung muss die Gegenstelle zur�ck auf 1200 Baud schalten, 
; um in diesem Modus bspw. den per Stub geladenen Code zu starten

		
ORG     0BF00h

START:  ; vollst�ndige Registersicherung (inkl. Shadow + IX/IY)
        PUSH    AF
        PUSH    BC
        PUSH    DE
        PUSH    HL
        ...
        DI                      ; im Aufrufkontext (ISR) korrekt: keine EI hier

        CALL    SETRUN          ; RUN-Konfig (wie bisher)
        CALL    RECV_ESCT       ; ESC 'T' empfangen und an Zieladresse schreiben
        CALL    SETCAOS         ; zur�ck auf CAOS-Konfig (wie bisher)

        ; vollst�ndige Registerwiederherstellung (umgekehrte Reihenfolge)
        EXX
        POP     HL
        POP     DE
        POP     BC
...
; Tabellen
;RUN_CTC:  DB 47h,17h ; 2400
;RUN_CTC:  DB 47h,0Bh ; 4800
RUN_CTC:  DB 47h,1h ; 56700
RUN_SIO:  DB 18h,02h,0E2h,14h,44h,03h,0E1h,05h,0EAh,11h,18h

CAOS_CTC: DB 47h,2Eh
CAOS_SIO: DB 18h,02h,0E2h,14h,44h,03h,0E1h,05h,0EAh,11h,18h